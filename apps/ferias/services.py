...
class FeriasExcelService:
    ...
    def gerar_relatorio_geral(ano):
        ...
        # Cabeçalhos com as novas colunas solicitadas
        headers_controle = [
            "Funcionário",
            ...
            "Período Aquisitivo",
            "Dias de Direito",
            "Férias tiradas no final do ano",        # NOVO
            "Férias tiradas no carnaval",            # NOVO
            "Faltas justificadas descontadas",       # NOVO
            "Total Dias Gozados",
            "Saldo",
            "Observações"
        ]
        ...
        periodos = (
            PeriodoAquisitivo.objects
            .select_related('funcionario', 'funcionario__dados_trabalhistas')
            .prefetch_related('ferias_registradas')
        ...
        for p in periodos:
            ...
            if hasattr(func, 'dados_trabalhistas'):
                ...
                admissao_contabilidade = getattr(dt, 'data_admissao_contabilidade', getattr(dt, 'data_admissao', None))
                admissao_marcenaria = getattr(dt, 'data_admissao_marcenaria', None)

            # --- CÁLCULO DOS NOVOS CAMPOS ---
            # Soma todos os registros de férias vinculados a este período aquisitivo
            ...
            dados = [
                func.nome,
                admissao_contabilidade,
                admissao_marcenaria,
                f"{p.data_inicio:%d/%m/%Y} a {p.data_fim:%d/%m/%Y}",
                p.dias_direito,
                total_recesso if total_recesso > 0 else "-",   # Valor Coluna Nova
                total_carnaval if total_carnaval > 0 else "-", # Valor Coluna Nova
                total_faltas if total_faltas > 0 else "-",     # Valor Coluna Nova
                p.dias_gozados(),
                p.saldo_restante(),
                obs
            ]
            ...
            formatos = [
                fmt['text'], fmt['date'], fmt['date'], fmt['center'], fmt['center'],
                fmt['center'], fmt['center'], fmt['center'], # Formatos novos campos
                fmt['center'], fmt['center'], fmt['text']
            ]
        ...
        pagamentos = (
            ...
            .select_related('funcionario', 'funcionario__dados_trabalhistas')
            .filter(data_pagamento__year=ano)
        ...
        # =====================================================
        # 3. RECIBOS DA CONTABILIDADE (REINSERIDO)
        # =====================================================
        row += 3
        ...
        # Lógica de proteção caso a tabela ainda não exista no banco
        try:
            recibos = RecibosContabilidade.objects.select_related('funcionario').all()
            row += 1
            for r in recibos:
